...
\begin{tikzpicture}[
    font=\sffamily,
    construct/.style={draw,thick,rounded corners,fill=orange!40,inner sep=.3cm, text width=17em, font=\large, minimum height=5em},
    mutate/.style={draw,thick,rounded corners,fill=black!10,inner sep=.3cm, text width=17em, font=\large, minimum height=5em},
    score/.style={draw,thick,rounded corners,fill=cyan!40,inner sep=.3cm, text width=17em, font=\large, minimum height=5em},
...
\node[construct] (crossover) at ($(score)+(0, -3)$) {\textbf{Using crossover}};
...
\draw (decision) edge[pil, out=-90, in=90] node {} (end);

\draw [->, ultra thick] (decision) -- ++(4, 0) -- ++(0, 9) -| node {} (score);
