...
\begin{itemize}
    ...
    of the LSTMs considered in this Chapter, and presents the different training
    data sets used to train them.
...
output layer~\ref{Krose1993, Jain1996}. The networks' input is a vector \(x\). The dimensionality, or
...
Frequently used activation functions~\cite{Krose1993, Karlik2011} include the:
...
error is called the \textit{loss function}. Loss functions include the
...
\cite{Ruder2016}. The gradient decent algorithm relies on the gradients. The gradients
...
Feed forward networks are a powerful technique when used for image recognitions
and many other applications. One of the networks' limitation is that there is no
memory associated with them, which is a problem for sequential data. RNN address that
issue by including a feedback look that lets them maintain information in
``memory'' over time. However, it can be difficult to train standard RNNs to
solve problems that require learning long-term temporal dependencies. This is
because the gradient of the loss function decays exponentially with time. LSTMs
are a type of RNN that use both a short-term and a long-term memory component.
The long-term memory component is the cell gate which as it was presented a set
of gates is used to control when information enters the memory, when it's
output, and when it's forgotten. This architecture lets them learn longer-term
dependencies and to be trained using the back propagation algorithm. This makes
LSTM are great tool for anything that has a sequence.

The networks were chosen to train a variety of IPD strategies because of their
ability to use a history of actions to make a prediction. The training of an
LSTM model in this Chapter is carried with the open source project Keras.
...
from the the collection of best response sequences, is transformed into 204
...
the pairs where the number of turns \(N \in [1, 204]\). For example
...
\hat{y} = & \begin{bmatrix} 1 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 & 0 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 & 0 & 0 \end{bmatrix} \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 & 0 & 0 & 0 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 & 1 & 1\end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 & 0 & 0 & 0 & 0\end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 & 1 & 1 & 0 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 1 & 1 & 0 & 0 & 0 & 0 & 0 \end{bmatrix} \nonumber \\
\vdots & & \vdots \nonumber
\end{align}
...
Two types of LSTMs have been trained in this Chapter. These are referred to:
...
training samples for the StoP network are in the form of
...
\hat{y} = & \begin{bmatrix} 1 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 0 \end{bmatrix} \nonumber \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 0 \end{bmatrix} \\
x = & \begin{bmatrix} 1 & 1 & 1 & 1 & 1 \end{bmatrix} &
\hat{y} = & \begin{bmatrix} 0 \end{bmatrix} \nonumber \\
...
workhorses of computer games, however, as stated by~\cite{Catanzaro2008}
...
NVIDIA's CUDA programming model~\cite{Harris2008} gave developers access and the
ability to express simple processing operations in parallel through code. In
2015, \cite{Steinkraus2005} presented a generic 2 layer fully connected neural
network GPU implementation which yielded to faster training over a CPU. In 2008,
\cite{Raina2009} was the first really important work to introduce GPUs to large neural
networks. The usage of GPUs in machine learning however was popularised in
2012 by~\cite{Krizhevsky2012}.
...
was overfitting, however, as an IPD strategy the outperforms any
...
Table~\ref{table:normalised_rank_lstm_tournaments} it is indicated that the
...
strategy ranks in the top half of a standard tournament? This is answered by
calculating the cumulative distribution function (CFD) of \(r\). CFD is the
cumulative probability for a given value. Thus, it can be used to to determine
the probability that a random observation that is taken from the population will
be less than or equal to a certain value. In the case here, that is the
probability that \(r < 0.5\). The CFDs are shown by
Figures~\ref{fig:cfd_s_to_s}-\ref{fig:cfd_s_to_p}.

For the StoS strategies trained on the entire training set there is a 0.78
...
of ranking in the top half of a tournament are 0.78, 0.76, 0.73 and 0.77.
Overall, the successful strategies covered
...
Ashlock fingerprints and the transitive fingerprints.
...
strategy. It is demonstrated that the strategies that had the same training
...
produced by the training presented in this Chapter the top LSTM strategies
are put against a list of opponents and their behaviour is explored more
in depth.

A list of six opponents were chosen. These are:

\begin{enumerate}
    \item Tit For Tat. Tif For Tat is a strategy that will retaliate a defecting
    but will also forgive is the opponent would apologise. The strategy was
    selected to see if the strategies exhibit such behaviour.
    \item Gradual. Gradual plays in a similar fashion as Tit For Tat but
    retaliates with a growing number of defection.
    \item Cooperator. A strategy that can be taken advantage from an opponent.
    It's selected to see if the LSTM players try to exploit such strategies.
    \item Defector. A strategy that just defect. It was selected to explore the
    LSTM strategies protect themselves from unconditional defections.
    \item ZDExtort2. A strategy that exploits is opponents. The strategy was
    chosen to see if the LSTM players protect themselves from being exploited.
    \item TF1. TF1 as presented in section is strategy that includes a
    handshake. The strategies are matched against TF1 to explore weather they
    have developed a handshake.
    \item Adaptive. A strategy explored in Chapter 6. Has a unique set of
    best responses, can be exploited to unconditionally cooperate while the
    opponent defect.
\end{enumerate}
...
deterministic one (following the opening turn) are performed very poorly. The
smallest \(\bar{r}\) has a value of 0.600. Thus the most successful strategy on
average performs on the bottom half of the tournaments it participated in.

%TODO include distributions?
...
to learn to use a history of a opponent's actions to make a prediction for 
the next move. The collection of best response sequences of
...
referred to as the sequence to sequence network and the sequence to probability
network. The two networks were presented in
section~\ref{section:training_a_rnn}. The two networks were trained on a
training data set generated by the collection of best response strategies,
but also on three subsets of the training
data set. This was done in order to understand the effect of the training data on the
network's performance in a IPD tournament. The subsets included the best response
sequences to top performing strategies, to strategies with ranks across a
standard tournament and a set of basic strategies. The networks were
...
The performance of the \lstmstrategies LSTM strategies were evaluated in
...
LSTM network type. Moreover, that the strategies trained on the subsets
...
case however for when the strategies made deterministic decisions based on the
LSTMs predictions.
...
cooperating rate compared to the rest of the trained strategies. This result
