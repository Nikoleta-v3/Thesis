...
Chapter~\ref{chapter:literature_review} presented a systematic literature review
on the PD. The Chapter aimed to summarise the results of the
established research, and to identify research topics in the field of the PD.
...
Chapter aims to contribute to the same research questions by using an automatic
approach. Namely, this Chapter presents a bibliometric analysis of 2,420 published
articles on the PD between 1951 and 2018. It presents the
topics in the PD publications, which have been identified using Latent
Dirichlet Allocation~\cite{Blei2003}, and it explores the changes in the
dominant topics over time.

Moreover, this Chapter explores the collaborative behaviour of the field of collaboration,
which is the third research question raised in this thesis,
using the co-authorship network. The network analysis is combined the Latent Dirichlet
Allocation topic analysis to also assess
the relative influence of authors in these topics.
...
the careers of junior researchers. This Chapter builds upon the previous work done
...
Several of the approaches described of this Chapter have previously been carried
out in~\cite{Bergmann2018,Liu2015,Sugimoto2011, youngblood2018}], the novelty is
combining the approaches as well as applying them to a new data set. More
specifically, in~\cite{Liu2015, youngblood2018}, a data set from a single
source, Web of Science, is considered whereas the data set described here,
archived at~\cite{pd_data_2018}, has been collected from five sources.
...
Python library called \textit{Arcas} which is available online~\cite{nikoleta_2017}.
Arcas includes documentation of usage which is available at: \url{http://arcas.readthedocs.io/en/latest/}. 
Arcas allows users to quickly create a query message to a publisher's API as
demonstrated in Figure~\ref{fig:arcas_query}. The API is specified by creating
an instance of a class. The names of the available api classes corresponds to
...
\end{minted}
\caption{Python Code. Example of using the library Arcas to create a message for
the API of the publisher PLOS. The query is for a single article with the
word `prisoners dilemma' in the title.}\label{fig:arcas_query}
\end{figure}

It can be seen that the query message for PLOS is different to that of arXiv which
was mentioned earlier. The message is used to query the API and retrieve a
reply. This is shown in Figure~\ref{fig:arcas_metadata}.
...
\end{minted}
\caption{Python Code. Example of using a query message to retrieve metadata.}
\label{fig:arcas_metadata}
\end{figure}

As mentioned previously, the structure of the received data differs from journal
to journal. The metadata data and their naming from PLOS is different from those
of arXiv. Arcas has a build in function to standarise the multiple types of replies,
Figure~\ref{fig:arcas_to_dataframe}.
...
The advantages of the library is that articles can be collected from a list of
APIs by specifying a single keyword. A total of four prominent publishers in the
field and a preprint server were used as sources to collect data for this
analysis:
...
More specifically, for this Chapter articles for which any of the terms
...
chosen for this Chapter and these are closeness and betweenness centrality.
...
In LDA, each document/abstract is represented by a distribution over topics, and
the topics themselves are represented by a distribution over words. More
specifically, each topics is described by weights associated with words and each
document by the probabilities of belonging to a specific topic. The probability
of a document belonging to a topic is referred to as the percentage contribution
denoted as \(c\). The output of an LDA is an \(N \times n\) matrix - \(N\) rows
for \(N\) abstracts and \(n\) columns for \(n\) topics. The cells contain the
percentage contributions for each topic for each abstract, \(c_i^ j\) for \(i
\in \{1, 2, \dots, n\}\) for \(j \in \{1, 2, \dots, N\}\). In essence, LDA maps
every paper to a vector space of dimension the number of topics.

For example the words and their associated weights for two topics A and B
could be:
...
are of interest. These papers include papers discussed in Chapter~\ref{chapter:literature_review}
such as~\cite{Flood1958} the first publication on the PD,~\cite{Ohtsuki2006, Stewart2012} two well cited
...
Five topics in the PD publications identified by the data set of this
...
\begin{table}[!hbtp]
    \centering
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/paper/bibliometric-study-of-the-prisoners-dilemma/assets/other_topics_network_comparison.tex}}
