...
\begin{center}
    ...
    Associated data sets: \cite{auction_data_2018, anarchy_data_2018, pd_data_2018}\\
    Associated codebase: \cite{nikoleta_2017} \\ \vspace{.5cm}
...
necessarily more collaborative than other scientific fields.
...
open source package Arcas which was used for the data collection. The Chapter
includes snippets of its source code, its unit tests and examples of its usage.
...
\begin{itemize}
    \item section~\ref{section:methodology} covers the data collection progress,
    an introduction to topic modelling and to co-authorship networks.
    ...
    of the field based on the publications of \authors authors.
\end{itemize}
...
order~\cite{sekara2018}, and to investigate the collaborative structure of an
...
between research fields. Understanding collaborative nature of a field is not
always an easy task. Several studies tend to consider academic citations as a
measure. A blog post published by Nature~\cite{nature_blog}
...
an author's collaborators, and clustering coefficient corresponds to the extent that
...
and extends their methodology as it will be described in
...
LDA is the topic modelling technique used in this thesis to identify research
...
fields have been done before~\cite{Bergmann2018,Liu2015,Sugimoto2011,
youngblood2018}. The novelty of this Chapter is the combination of these
techniques and their application to a new data set. A data set that has been
collected from five different sources, whereas the data sets of previous
works~\cite{Liu2015,youngblood2018} were from a single source (Web of Science).
A bespoke open source software was used to carry out the data collection.
Details of the software and the data collection process are presented in
...
dilemma&max_results=1}

represents a request message. The first part of the
...
In this example it is requested a single article that the word ``Prisoner's Dilemma'' exists within
...
APIs has been packaged as a Python library called Arcas. The source
...
differences between the requesting and receiving phases of the APIs.

Consider the example of retrieving a single article with the word ``Prisoner's
Dilemma'' in the title. Figure~\ref{fig:arcas_query_plos} demonstrates the Python
code needed to query the publisher PLOS and Figure~\ref{fig:arcas_query_nature}
demonstrates code for querying the API of Nature. The only distinction between
the two code snippets is their respective line 2 where the API is specified by
creating an instance of a class corresponding to the publisher's API. The
differences between querying the two APIs are visible from lines 6 and
14-onwards. Lines 6 show the requesting message and lines 14-onwards show the
metadata of the article received by each source.
...
PLOS. The query is for a single article with the word ``Prisoner's Dilemma'' in
...
\end{usagepy}
\caption{Example of using the library Arcas to communicate the API of the publisher
Nature. The query is for a single article with the word ``Prisoner's Dilemma'' in
the title.}\label{fig:arcas_query_nature}
\end{figure}

There are differences and similarities between the retrievable metadata of each
API. Arcas includes a function which standarises the format of querying results.
Figure~\ref{fig:arcas_to_dataframe} demonstrates the usage of the function.
...
At the time of writing there are a total of five different APIs implemented
within the project. These five include APIs of four prominent publishers in the
field and a preprint server. Namely these are:
...
Each API has a corresponding class implemented in Arcas. The classes include
...
publisher's website and for the APIs of this Chapter they can be acquired for free.
...
class Arxiv(Api):
    ...
    def to_dataframe(self, raw_article):
        ...
        transforms it to a standardised format and returns a dataframe."""
    ...
    def get_root(response):
        ...
        return root
\end{minted}
\caption{\mintinline{python}{Arxiv} class implementation in Arcas. It includes the code necessary for Arcas
...
\end{minted}
\caption{\mintinline{python}{Ieee} class implementation in Arcas. It includes the code necessary for Arcas
...
method \mintinline{python}{to_dataframe} of the \mintinline{python}{Arxiv} class. Moreover, Figure
...
\end{sourcepy}
\caption[Unit tests for the \mintinline{python}{Arxiv} class.]{Unit tests for the \mintinline{python}{Arxiv} class.
Tests the functionality of the method \mintinline{python}{to_dataframe}.}\label{fig:test_arxiv}
\end{figure}
...
def test_parameters_and_url_author():
    ...
    assert (
    url ==
    "https://ieeexploreapi.ieee.org/api/v1/search/articles?author=Glynatsi&apikey=Your key here"
    )
...
\end{sourcepy}
\caption{Unit tests for the \mintinline{python}{Ieee} class.}\label{fig:test_ieee}
\end{figure}


The \totalarticles articles metadata explored in this Chapter has been collected
...
\begin{enumerate}
    ...
    A representative of this idea is \textit{closeness centrality}, where a node
    ...
    \item Another centrality measure is the \textit{betweenness centrality},
...
designed to summarise large collections of documents by a small number of
...
More specifically, each topic is described by weights associated with words.
For example assume two topics A and B. Their associated words and weights
are:
...
A document with abstract ``The study of
cooperation in humans'' has a \(c_{A} = 0.039 + 0.028 + 0.026 = 0.093\) and
\(c_B = 0.020 + 0.0 + 0.0 = 0.020\). In essence, LDA maps every paper to a
vector. In this example the document is mapped to \([0.093, 0.020]\). Each
document has a dominant topic. The dominant
topic is the topic with the highest percentage contribution denoted as \(c^*\).
For the given example the dominant topic is Topic A, and \(c^*=c_A\).
...
keywords between two topics. Both these approaches will be used here.
Preceding the analysis of research topics, the next section presents a
preliminary analysis of the collected data set.
...
articles in the field, and a series of works of Axelrod
\cite{Axelrod1980a, Axelrod1980b, Axelrod1981, Axelrod1987, Riolo2001}.
...
from arXiv with 11 articles per year, followed by Springer with 9, and PLOS
and Nature with 8.

\begin{table}[!hbtp]
    \begin{center}
    \resizebox{.9\textwidth}{!}{
    \input{src/chapters/03/tex/preliminary_table}}
    \end{center}
...
\begin{enumerate}
    ...
    1970s.
    \item There is a decrease in 2017-2018. This is due to the data set being
...
be seen that Perc is an outlier. More specifically, most authors have
1 to 6 publications in the data set.

\begin{figure}[!hbtp]
    \centering
    \includegraphics[width=.50\textwidth]{src/chapters/03/img/papers_per_author.pdf}
    \caption{Distribution of number of papers per author (on a log scale).}
    \label{fig:num_papers_per_author}
\end{figure}

The overall collaboration index or the average number of authors on
...
corresponds to the 22\% of papers. It appears that academic publications
tend to be undertaken in the form of collaborative effort, which is in line
with the claim of~\cite{Kyvik2017}. From
Figure~\ref{fig:ci_over_time} the trend of collaboration index over the years is given. There are
...
are overlapping keywords between the topics.
Further manual investigation has
revealed that the separation of topics are the most clear when an \(n\) of \(5\)
is considered. The LDA model for \(n=5\) has a coherence value 0.406 which is
close to 0.418 (the score for \(n=6\)). Thus, \(n=5\) is chosen to carry out
the LDA.

\begin{table}[!hbtp]
    \begin{center}
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/topics_keywords_n_6.tex}}
    \end{center}
...
\begin{itemize}
    ...
    representative article, Topic A appears to be about human subject
    ...
    participants,~\cite{Li20141} which studied the difference in cooperation
    between high-functioning autistic and typically developing
    children,~\cite{Molina2013} explored the gender effect in high school
    ...
    is focused on biological studies. Papers assigned to the topic include
    ...
    C appears to include publications on PD strategies. Publications
    in the topic include the introduction of new strategies~\cite{Stewart2013},
    ...
    is focused on evolutionary dynamics on networks. Publications include
    ...
    the fixation probabilities of any two strategies in spatial
    structures.
    \item The publications assigned to Topic E are on modelling problems
...
\begin{table}[!hbtp]
    \begin{center}
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/documents_of_topic}}
    \end{center}
...
Note that whilst for the choice of 5 topics the actual clustering is not
...
its publications have been increasing. There is no decreasing
trend in any of the topics. All topics have been publishing for years and
...
\begin{table}[!hbtp]
    ...
    \input{src/chapters/03/tex/topics_per_year_table}}
    \end{center}
...
however are small. The relevance of the five topics has been increasing
...
\begin{figure}[!hbtp]
    ...
    and the optimal number of topics over time. The optimal number of topics are
    based on the coherence score.}
...
referred to as the main cluster of the network and is denoted as \(\bar{G}\). A
...
\begin{table}[!hbtp]
    \centering
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/network_comparison.tex}}
...
total of 3,444 publications have been collected for auction games and 748 for
price of anarchy. Price of anarchy is a relatively new field, with the first
...
\begin{table}[!hbtp]
    \centering
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/other_topics_network_comparison.tex}}
...
\begin{table}[!hbtp]
    \centering
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/cumulative_clusters_table.tex}}
...
\begin{table}[!hbtp]
    \centering
    \resizebox{\textwidth}{!}{
    \input{src/chapters/03/tex/topic_network_comparison.tex}}
...
number of isolated nodes is very small \( < (0.2)\) compared to Topic E where the
...
\begin{table}[!hbtp]
    \begin{center}
    \resizebox{.9\textwidth}{!}{\input{src/chapters/03/tex/centralities_pd_graph.tex}}
\end{center}
\caption{The 10 most central authors based on betweenness and closeness centralities
...
\end{center}
\caption{The 10 most central authors based on betweenness centrality
...
\end{center}
\caption{The 10 most central authors based on closeness centrality
...
The distributions of both centrality measures for all the networks are given in
the Appendix~\ref{appendix:distributions}.

\section{Chapter summary}

This Chapter explored the research topics from a collection of \totalarticles
publications of the IPD, and moreover, the authors' collaborative behaviour and
their influence in the research field. This was achieved by applying network
theoretic approaches and a LDA algorithm to the collection of publications. Both
the software~\cite{nikoleta_2017} and the main data set~\cite{pd_data_2018}
associated with the Chapter have been archived and are available to be used by
other researchers. In fact Arcas has been used by~\cite{brane}
and~\cite{arcas_blog}.

Arcas, its development and the data collection were covered in section~\ref{section:methodology},
...
publishing. The LDA analysis showed that the articles could be classified into 5
...
problems as a PD. These topics summarise the field of the PD well, as they
