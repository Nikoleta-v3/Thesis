...
def get_model_and_opt_num_of_topics(yearly_dataset, limit, step=1, start=2):
    ...
    model_list, coherence_values = tools.compute_coherence_values(
        limit=limit,
...
def get_dataframe(ldamodel, corpus):
    ...
    for i, row in enumerate(ldamodel[corpus]):
        ...
        for j, (topic_num, prop_topic) in enumerate(row):
            if j == 0:
                ...
                data_with_topics = data_with_topics.append(
                    pd.Series(
...
for year in tqdm.tqdm(periods):
    ...
    model, index, corpus = get_model_and_opt_num_of_topics(
        yearly_data, limit=15, start=3
    )
