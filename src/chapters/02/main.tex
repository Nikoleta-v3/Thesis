...
(\TitForTat) of the first computer tournament, run by Robert Axelrod.
...
similarly to \TitForTat it is used in tournaments today and has
...
mutual cooperation again, the only way \TitForTat would end up in \(DC\), in
environments without noise, is if it had received a defection and then
...
of the game. It
was shown that for
a sufficient high \(w\) \TitForTat strategies
...
archetypes. The top three strategies were \EvolvedLookerUpTwoTwoTwo,
\EvolvedHMMFive and \EvolvedFSMSixTeen.
