...
behavioral tendencies of their counter party. In the IPD defecting is no
...
\begin{figure}[!hbtp]
    \centering
    \includestandalone[width=\textwidth]{src/chapters/01/thesis_structure}
...
\begin{itemize}
    ...
    It presents the top performing strategies and analyzes their salient features.
...
by analysing a data set of tournaments and evaluating the performance of pre
designed strategies. The exact opposite is done in
...
\begin{figure}[!hbtp]
    \centering
    \includestandalone[width=.9\textwidth]{src/chapters/01/depth_structure}
...
\begin{figure}[!hbtp]
    \centering
    \includegraphics[width=.8\textwidth]{src/chapters/01/GitHub_discussion}
...
with the programing language Python. Anaconda is a free and open-source
...
dependencies:
  ...
  - pip:
    ...
    - fsspec==0.3.3
\end{shell}
\caption{An example of an environment file. These are saved with the extension
\mintinline{shell}{yml}. The name of the specific environment is called
...
of code behave as expected, and the test the application from thr point of the
