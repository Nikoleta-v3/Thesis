...
\begin{center}
    ...
    Axelrod-Python library (APL) version: 3.0.0 \\ \vspace{.5cm}
...
to the PD and the discussion of previous literature. Both the introduction to
the PD and the discussion of previous tournaments and strategies are excluded
from the Chapter. That is because the introduction to the PD was presented
in Chapter~\ref{chapter:introduction} and the previous literature was discussed
in Chapter~\ref{chapter:literature_review}.

\newpage

\section{Introduction}

In Chapter~\ref{chapter:introduction} it was discussed that conceptualising strategies and
...
computer tournaments. Furthermore, a large portion of these strategies are drawn from the
...
performance of the strategies using modern machine learning and statistical techniques. These
...
Axelrod on properties of successful strategies (presented in
...
\begin{itemize}
    ...
    \item It is okay to be clever
    \item Adapt to the environment; Adjust to the mean population cooperation rate
\end{itemize}
...
of strategies that have been contributed specifically to the package. A total
of \numberofstrategies strategies are used in this Chapter, a list of these
is given in the Appendix~\ref{app:list_of_players}.
...
Algorithm~\ref{algorithm:data_generation}.

\begin{algorithm}[!htbp]
    \setstretch{1.35}
    \ForEach{\text{seed} $\in [0, 11,400]$}{
    ...
    \caption{Tournaments' result summary collections algorithm}
    \label{algorithm:data_generation}
\end{algorithm}

For each trial a random size \(N\) is
...
\begin{table}[!htbp]
    ...
    \bottomrule
        \end{tabular}}
    \end{center}
    \caption{Data collection; parameter values.}
...
Table~\ref{table:output_result}. These have been archived and are available
at~\cite{Glynatsi2019_meta, Glynatsi2019_meta_raw_data}.
Each strategy has participated on average in
5,154 tournaments of each type. The strategy with the maximum participation in each
tournament type is \InversePunisher with 5,639 entries. The strategy with the
minimum entries is \EvolvedLookerUpOneOneOne which was selected in 4,693 trials.
...
the same type (on average 5,154). For example \TitForTat participated in a
total of 5,114 tournaments of each type. The strategy's normalised rank
...
\begin{table}[!htbp]
    \begin{center}
    \resizebox{\textwidth}{!}{
        \input{src/chapters/04/paper/meta-analysis-of-prisoners-dilemma-tournaments/paper/top_perfomances.tex}
    }
\end{center}
\caption{Top performances for each tournament type based on $\bar{r}$. The
results of each type are based on 11,420 unique tournaments. The
results for noisy tournaments with \(p_n < 0.1\) are based on 1,151 tournaments,
and for probabilistic ending tournaments with \(p_e < 0.1\) on 1,139. The top
...
in~\cite{Knight2017}. These are strategies based on finite state automata (FSM),
...
strategies not from the literature but from APL. \DoubleCrosser is an extension
...
impact of the PD being iterated is subdued. As stated by the Folk
...
\begin{table}[!htbp]
    ...
    \input{src/chapters/04/tex/performance_merged_table.tex}}
    ...
    difference. \PSOGamblerTwoTwoTwoNoiseZeroFive and \EvolvedHMMFive are trained strategies introduced
    in~\cite{Knight2017} and \NiceMetaWinner and \NMWEMemoryOne are strategies
...
Though there is not a single strategy that repeatedly outranks all others in any
of the distinct tournament types, or even across the tournament types, there
are specific types of strategies have been repeatedly ranked in the top ranks.
These have been strategies that have been trained, strategies that defect,
...
$k$ & The number of repetitions of a given tournament & trial summary & integer & 10 & 100 \\
    ...
    strategy is behaving as a ZDs. The rest of the features considered include the
...
\begin{table}[!htbp]
    \begin{center}
    \resizebox{.9\textwidth}{!}{
        \input{src/chapters/04/paper/meta-analysis-of-prisoners-dilemma-tournaments/paper/correlations_table.tex}
    }
\end{center}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=.7\textwidth]{src/chapters/04/paper/meta-analysis-of-prisoners-dilemma-tournaments/images/rates_of_winners_in_standard_tournaments.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=.7\textwidth]{src/chapters/04/paper/meta-analysis-of-prisoners-dilemma-tournaments/images/c_r_winners_tournaments.pdf}
...
The probability of noise has been observed to substantially affect successful
behaviour.
...
clear that cooperating only 67\% of the time the mean cooperator did is
...
opponents. The winners of the tournaments considered in this analysis are
envious, but not as much as many ZDs.
Though the exact interactions between the matches have not been recorded here,
the work of~\cite{Knight2017} which introduced the trained strategies that
appeared in the top ranked strategies of section~\ref{section:top_performances}
did. In~\cite{Knight2017} it was shown that clever strategies managed to achieve
...
be the reason why ZDs fail to appear in the top ranks -- they try to exploit
...
Based on the results of~\cite{Knight2017} this could be because they are
...
in line with the results of~\cite{Bendor1991, Donninger1986, Molander1985,
...
In this Chapter well performed behaviour was explored whilst considering a collection
of pre defined strategies. In comparison, Chapter~\ref{chapter:memory_one} does
not consider pre defined strategies but estimates the exact best responses. This
is done by considering strategies with a theory of mind that compete in
environments of memory-one opponents.
