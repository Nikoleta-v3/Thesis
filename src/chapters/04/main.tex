...
\begin{center}
    ...
    Associated data set: \cite{Glynatsi_2019_metatournament} \\ % TODO add raw data citation
...
alternative settings against known powerful strategies. %TODO rewrite and just describe the novelty of this chapter

The aim of this Chapter is to perform a similar analysis and to revisit the
properties proposed by Axelrod which have in turn influenced several
publications across the years.

Compared to many of these publications (many reviewed in
Chapter~\ref{chapter:literature_review}) where typically a few selected or
introduced strategies are evaluated on a small number of tournaments and/or
small number of opponents, this Chapter evaluates the performance of
\numberofstrategies strategies in \numberofalltournaments tournaments. Many of
the \numberofstrategies strategies are known strategies from the literature and
include many previous tournament winners, in contrast to other works that may
have randomly generated a collection of essentially arbitrary strategies
(typically restrained to a class such as memory-one strategies, finite state
machines or deterministic memory two strategies). Additionally, the analysis of
this Chapter considers tournament variations including
standard tournaments, tournaments with
noise, probabilistic match length, and both noise and probabilistic match
length. This diversity of strategies and tournament types yields new insights
and tests earlier claims in alternative settings against known powerful
strategies.
...
\section{Chapter Summary}\label{section:conclusion} %TODO re write
