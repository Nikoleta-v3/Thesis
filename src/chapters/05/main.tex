...
This Chapter explores best reponse memory-one strategies. In a similar fashion
to~\cite{Press2012} a given memory-one strategy is considered; however,
whilst~\cite{Press2012} found a way for a player to manipulate a given opponent,
this Chapter will consider a multidimensional optimisation approach to identify
the best response to a given group of opponents. The outcomes of this thesis
reinforce known results which were presented in
...
In particular, the Chapter presents a compact method of identifying the best
...
\begin{itemize}
    \item section~\ref{section:utility} defines the analytical formulation of
    ...
    of opponents.
    \item section~\ref{section:resultant_theory}, reviews resultant theory.
    \item section~\ref{section:stability_of_defection}, presents a compact method of identifying environments
    for which coopertion in known to not occur.
...
denoted as \(U_q(p)\), has been calculated Axelrod-Python~\cite{axelrodproject}.
...
memory-one strategies and in section~\ref{section:stability_of_defection}
...
\input{src/chapters/05/tex/proof_theorem_three.tex}
...
\section{Reactive Strategies \& Resultant Theory}\label{section:resultant_theory}

Reactive strategies are a subset of memory-one strategies, described in
Chapter~\ref{chapter:literature_review}. As a reminder, reactive strategies only
take into account the opponent's previous moves and thus can be written as a
vector in \(R^2\). Thus, the optimisation problem of
(\ref{eq:mo_tournament_optimisation}) now has an extra constraint and is re
written as,
...
Reactive strategies allow for best responses to be studies as a problem in \(R^2\),
and the Theorem of can.
...
of degree \(n\) with roots \(a_i, i=1, \dots, n\) and a polynomial

\[ g(x) = b_m x ^m + b_{(m-1)} x^{(m-1)} + \dots + b_1 x + b_0 \]

of degree m with roots \(b_j, j=1, \dots , m,\) the resultant denoted as \(R(f, g)\)
and also called the eliminant~\cite{Salmon1924}, is defined by

 \[ R(f, g) = a_n^m b_m^n \prod_{(i=1)}^n \prod_{(j=1)}^m( a_i - b_j)  \]

The resultant is widely used in number theory, either directly or through the
discriminant, The resultant is also given by the determinant of the
corresponding Sylvester matrix~\cite{Akritas2014}. The Sylvester matrix
associated to \(f\) and \(g\) is the \((n+m)\times (n+m)\) matrix constructed as
follows:
...
actually allow us to find the roots of the system. This methodoloyg is also
...
The resultants of The resultant of n homogeneous polynomials in n variables (also called
