...
project did not be include the feature to calculate multivariate resultants. As
...
Best responses will be estimated heuristically using Bayesian
...
Let \(x_i\) be a maximizer of the acquisition function over \(x\), where the acquisition function is computed using the current posterior distribution\;
Observe \(y_i\) = \(f(x_i)\)\;
Increment \(i\)\;
}
...
random memory-one opponents. This is done in both tournaments with and without
self interactions. The extortionate behaviour of these best responses is then
evaluated using the SSE method presented in this section. Finally,
section~\ref{subsection:longer_memory_best_response} compares the performances
of memory-one and longer-memory best responses against a number of opponents.
...
A data set of best response memory-one strategies with \(N=2\) opponents has
been generated which is available at~\cite{glynatsi2019}. The data set contains
...
Similarly to previous section, a large data set has been generated with
...
game, and more specifically, memory-one best responses.

\begin{itemize}
    \item It has proven that the utility of a memory-one strategy against a set
